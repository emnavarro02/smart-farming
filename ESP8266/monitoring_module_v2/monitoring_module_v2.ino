 ...
 *     
 *  .CHANGE LOG
 *   14-DEC-2018: Reduced delay time to avoid timeout. Performed some aditional cosmetic changes and 
 *                optimizations
 *               
 */  
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // Decode JSON request
  StaticJsonBuffer<200> callbackJsonBuffer;
  JsonObject& data = callbackJsonBuffer.parseObject((char*)json);
  ...
  methodName = "";
  callbackJsonBuffer.clear();
}

String get_gpio_status() {
  // Prepare gpios JSON payload string
  StaticJsonBuffer<200> getGpiojsonBuffer;
  JsonObject& data = getGpiojsonBuffer.createObject();
